...
public interface IProduitRepository extends JpaRepository<Produit, Long>{
    ...
    @Query("SELECT p.idProduit, p.numSerie, p.designation, p.categorie.nom, p.categorie.prixNormal, p.categorie.prixPromotionel, p.etat, p.remarque FROM Produit p WHERE p.situation=:x")
    public Page<Object[]> findBySituation(@Param("x") Situation situation, Pageable pageable);
    @Query("SELECT p.idProduit, p.numSerie, p.designation, p.categorie.nom, p.categorie.prixNormal, p.categorie.prixPromotionel, p.etat, p.remarque, p.categorie.idCategorie, p.situation FROM Produit p WHERE p.idProduit=:x")
